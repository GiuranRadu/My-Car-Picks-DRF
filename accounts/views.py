...
def cookie_token_obtain_pair(request):
    ...
    response = Response({"message": "Logged in", "user": user_data}, status=200)
    ...
    create_cookie_fn(
        ...
        new_value=refresh_token,
        max_age=REFRESH_MAX_AGE,
    )
    create_cookie_fn(
        response=response,
        cookie_name="cookie_nostru",
        new_value="salut frate",
        max_age=20,
        httponly=False,
    )
    create_cookie_fn(
        response=response,
        cookie_name="mambo_number_2",
        new_value="lou bega",
        max_age=20,
        httponly=False,
    )
